...
- **Update 11 (2026-06-24) — Engine & infra fixes dari review eksternal:**
  - Fix: **Clash/interaction coloring kini berbasis Yong Shen, bukan tipe interaksi semata** — `annotate_favorability()` baru di `interactions.py` membandingkan elemen branch natal yang terdampak terhadap Yong Shen chart (via `get_element_relation()` baru di `calculator.py`); tag `favorability` ('challenging'/'favorable'/'neutral'/null) dikirim di setiap interaction object dari `/calendar/current`, `/calendar/date`, `/calendar/narasi`, `/calendar/annual`, `/calendar/energy-summary`; field baru `InteractionSchema.favorability`. Sebuah Clash yang mengenai branch yang BUKAN Yong Shen sekarang ditandai 'favorable' (teal) bukan otomatis 'challenging' (terracotta) — fallback ke logika lama berbasis tipe jika Yong Shen belum resolve ("需要判断")
  - Fix: **CalendarScreen** — `getInteractionDisplay()` & `getEnergyStatus()` direfactor untuk membaca `favorability`; teks plain/antidote punya varian khusus saat interaksi disruptif ternyata favorable (membersihkan elemen yang tidak dibutuhkan chart) atau six_combination ternyata neutral (tidak memperkuat Yong Shen); `INTERACTION_META` dihapus, diganti `INTERACTION_BASE` (label/icon saja, warna kini dinamis dari favorability)
  - Fix: **Stem Combination (天干合) kini mempengaruhi Day Master Strength** — `calculate_day_master_strength()` di `calculator.py` mendeteksi kombinasi stem antar year/month/hour, dan jika elemen hasil kombinasi didukung/diproduksi oleh elemen Month Branch (syarat klasik 合化), kontribusi skor kedua stem tersebut beralih ke elemen hasil kombinasi. Day stem dikecualikan (transformasi Day Master/化氣格 di luar scope engine ini). Diverifikasi unit test manual: skor naik 1 tingkat saat transformasi terpenuhi vs tidak
  - Fix: **`hour_unknown` kini diteruskan ke AI** — `_build_chart_dict()` menambah field `hour_unknown`; `PROFILE_SYSTEM_PROMPT` dan `PROFILE_SYSTEM_PROMPT_V2` di `cerebras.py` diberi instruksi eksplisit untuk menyisipkan catatan "estimasi dasar" di bagian Element Balance saat jam lahir tidak diketahui, supaya AI tidak terdengar terlalu percaya diri pada data yang sebagian estimasi
  - Feat: **`GET /api/health`** — liveness probe baru di `router.py`; dipanggil fire-and-forget oleh `ChartContext.tsx` saat app startup untuk memicu wake-up HF Spaces (free tier sleep setelah idle) sebelum user benar-benar membuka tab Kalender
  - Fix: **`bazi_prompt.md` disinkronkan ulang** — dokumen sebelumnya masih mendeskripsikan `PROFILE_SYSTEM_PROMPT` v1 padahal runtime sudah pakai `PROFILE_SYSTEM_PROMPT_V2` (format `SECTION:`); sekarang mencantumkan prompt v2 + aturan `hour_unknown` + field `favorability` + tabel endpoint lengkap termasuk `/calendar/energy-summary` dan `/health`
  - Skip (dengan sengaja): rekomendasi auth/orphaned-data cleanup dari review eksternal — tidak relevan, app ini dipakai personal ("Self App"), bukan produk multi-user
  - Feat: **Context-aware Yong Shen / Ge Ju tooltip** — `getPersonalizedTermBody()` baru di `ProfileScreen.tsx` menyisipkan paragraf praktis tambahan ke modal `[i]` Yong Shen dan Ge Ju, dihitung dari chart pengguna sendiri (bukan kamus statis generik). Yong Shen disimpan sebagai kategori Ten God (官殺/印綬/財星/比劫/食神), bukan elemen mentah — `getElementRelation()`/`resolveYongShenElement()` di frontend mem-mirror logika `get_element_relation()` backend untuk mencari elemen konkret (Kayu/Api/Tanah/Logam/Air) dari kategori + elemen Day Master, lalu dipetakan ke `ELEMENT_PRACTICAL` (saran aktivitas per elemen). Ge Ju langsung dipetakan ke `GE_JU_PRACTICAL` (saran aktivitas per salah satu dari 10 struktur)
  - Fix: **Yong Shen tooltip kini memvalidasi kondisi sebelum memberi saran** — `YONG_SHEN_STRENGTH_LEAD` baru bercabang berdasarkan `day_master_strength` (Strong/Moderate-Strong → framing "energi berlebih butuh saluran keluar"; sisanya → framing "kehabisan dukungan, memaksa diri jalan terus"), sebelum baris saran per-elemen. Sebelumnya teks generik sama persis dikirim untuk elemen yang sama terlepas dari KENAPA elemen itu jadi Yong Shen — dua kondisi yang secara emosional sangat berbeda dapat saran yang identik
  - Fix: **Data BaZi yang sudah dikumpulkan tapi tidak terpakai — diaudit dan disambungkan semua:**
    - `THREE_COMBINATIONS` (三合局) di `tables.py` sudah ada sejak lama tapi tidak pernah dipanggil di mana pun. `detect_three_combinations()` baru di `interactions.py` mendeteksi bureau elemen "full" (3/3 branch hadir) atau "partial" (2/3); `calculate_day_master_strength()` di `calculator.py` ikut ditransformasi (full selalu transform, partial hanya jika Pilar Bulan ikut terlibat) — diverifikasi: skor naik dari Moderate ke Strong saat 三合局 lengkap terbentuk
    - `THREE_PENALTIES` di `tables.py` juga dead-import di `calculator.py` — `_PENALTY_PAIRS` dan `_SELF_PENALTY_BRANCHES` di `interactions.py` sekarang DIDERIVE programatik dari `THREE_PENALTIES` (bukan daftar manual terpisah yang berisiko desync)
    - `detect_natal_internal_interactions()` baru — Clash/Combination/Harm/Penalty ANTAR pilar natal sendiri (mis. Pilar Tahun clash Pilar Jam) dibaca sebagai sifat bawaan, bukan kejadian waktu. Tidak dianotasi `favorability` (bukan event harian)
    - `detect_luck_pillar_interactions()` baru — interaksi antara branch Luck Pillar (大運) aktif vs pilar natal; sebelumnya dekade aktif tidak pernah ikut perbandingan interaksi sama sekali, hanya kalender harian/tahunan
    - Refactor: `_classify_branch_pair()` helper baru menyatukan logika Clash/Combination/Harm/Penalty yang dipakai bersama oleh `detect_calendar_interactions`, `detect_luck_pillar_interactions`, dan `detect_natal_internal_interactions` — sebelumnya logika ini cuma ada sekali di `detect_calendar_interactions`
    - `special_stars` dan `pillar_life_stages` sudah dihitung & ditampilkan di UI lama, tapi tidak pernah masuk ke payload AI (`_build_chart_dict`) — sekarang ditambahkan, beserta `three_combinations`, `natal_interactions`, `active_luck_pillar_interactions`, dan `life_stage` pada `active_luck_pillar` (sebelumnya hanya ada di schema response untuk UI, hilang di payload AI)
    ...
    - Frontend: section baru "三合局 THREE COMBINATIONS" dan "INTERAKSI INTERNAL CHART" di `ProfileScreen.tsx` (gaya konsisten dengan Stem Combinations existing), badge hubungan Luck Pillar aktif vs natal di bawah scroll Luck Pillars; `TermKey`/`TERM_EXPLANATIONS` ditambah `three_combo` dan `natal_interactions` dengan teks Mode Awam yang konsisten
  - Fix: **Bug regresi dari refactor `_classify_branch_pair` (ditemukan saat self-audit)** — versi awal cuma return SATU klasifikasi per pasangan branch (match pertama menang: clash→combo→harm→penalty), padahal beberapa pasangan branch klasik masuk ke DUA kategori sekaligus (mis. 寅-巳 itu Six Harm SEKALIGUS bagian dari Ungrateful Penalty group). Versi lama (pre-refactor) mendeteksi keduanya independen via loop terpisah; refactor awal diam-diam menghilangkan salah satunya. `_classify_branch_pair()` sekarang return `list[dict]` (bisa lebih dari satu match), tiga caller (`detect_calendar_interactions`, `detect_luck_pillar_interactions`, `detect_natal_internal_interactions`) diupdate untuk iterate semua match — diverifikasi manual: 寅-巳 sekarang correctly menghasilkan dua interaction (`harm` + `penalty`)
  - Fix: **Makna klasik Clash/Harm/Penalty/Self-Penalty diperbaiki — sebelumnya disederhanakan keliru:**
    - **Harm (六害)** sebelumnya digambarkan sebagai "gosip/dinamika tersembunyi" — ini bukan makna klasik. Diperbaiki jadi: kerugian tersembunyi dari pihak yang seharusnya dekat/membantu (dikhianati tanpa konfrontasi terbuka, bukan soal gosip)
    - **Penalty (三刑)** sebelumnya satu teks generik "tekanan eksternal" untuk semua subtipe — padahal punya 3 subtipe klasik dengan makna sangat berbeda (Ungrateful 無恩之刑 = dikhianati setelah membantu, Bullying 持勢之刑 = penyalahgunaan kekuasaan, Uncivilized 無禮之刑 = sengketa soal batasan/hormat). Backend sekarang mengirim field terstruktur `penalty_name` (bukan cuma terselip di string `description`); frontend (`PENALTY_SUBTYPE` di `CalendarScreen.tsx`) punya teks `plain`/`antidote` per subtipe, termasuk varian "favorable" masing-masing
    - **Clash** dan **Self-Penalty** sudah cukup akurat sebelumnya, hanya direfine sedikit kata-katanya
    - Per-tipe `favorable` copy (dari fix sebelumnya yang masih generik untuk semua 4 tipe disruptif) ikut diperbaiki mengikuti makna yang sudah dikoreksi
  - Feat: **`bazi-app/src/baziTerms.ts`** — modul shared baru, single source of truth untuk `PENALTY_NAME_ID` (label Indonesia singkat per subtipe penalty) dan `getInteractionDetail(type, penaltyName?)` (penjelasan makna klasik panjang, dipakai CalendarScreen DAN ProfileScreen agar tidak drift)
  - Feat: **Tombol "ⓘ Detail makna klasik"** — setiap interaction card di `CalendarScreen.tsx` (kalender harian) dan setiap baris di "INTERAKSI INTERNAL CHART"/hubungan Luck Pillar di `ProfileScreen.tsx` sekarang punya tombol detail yang membuka `InfoModal` dengan penjelasan klasik penuh, dipisah dari teks ringkas/antidote yang selalu tampil
